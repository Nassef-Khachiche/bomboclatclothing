...
function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [dashboardRes, ordersRes, customersRes, inventoryRes, productsRes, collectionsRes] = await Promise.all([
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        ...
        <Panel title="Collections" items={collections.map((collection) => `${collection.name} (${collection._count.products})`).slice(0, 8)} />
